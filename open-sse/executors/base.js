    ...
    for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
      ...
      try {
        ...
        if (
          ...
          retryAttemptsByUrl[urlIndex] < RETRY_CONFIG.maxAttempts
        ) {
          retryAttemptsByUrl[urlIndex]++;
          const delay = retryAfterMs || RETRY_CONFIG.delayMs;
          log?.debug?.(
            "RETRY",
            `429 retry ${retryAttemptsByUrl[urlIndex]}/${RETRY_CONFIG.maxAttempts} after ${Math.ceil(delay / 1000)}s${retryAfterMs ? " (Retry-After)" : ""}`,
          );
