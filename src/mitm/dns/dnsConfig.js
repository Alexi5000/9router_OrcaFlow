  ...
  try {
    if (IS_WIN) {
      ...
      for (const host of entriesToAdd) {
        ...
        scriptLines.push(
          `    Add-Content -Path $hostsPath -Value '127.0.0.1 ${host}' -Encoding UTF8 -ErrorAction Stop`,
        );
...
async function removeDNSEntry(tool, sudoPassword) {
  ...
  try {
    if (IS_WIN) {
      ...
      scriptLines.push(`try {`);
      scriptLines.push(
        `  Copy-Item -Path '${tmpEsc}' -Destination '${hostsEsc}' -Force -ErrorAction Stop`,
      );
    ...
    console.log(
      `✅ Removed DNS entries for ${tool}: ${entriesToRemove.join(", ")}`,
    );
  } catch (error) {
    const msg = error.message?.includes("incorrect password")
      ? "Wrong sudo password"
...
async function removeAllDNSEntries(sudoPassword) {
  for (const tool of Object.keys(TOOL_HOSTS)) {
    try {
      ...
      console.log(
        `[MITM] Warning: failed to remove DNS for ${tool}: ${e.message}`,
      );
    }
  }
}
