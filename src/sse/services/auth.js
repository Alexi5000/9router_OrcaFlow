...
export async function getProviderCredentials(
  ...
  try {
    ...
    log.debug(
      "AUTH",
      `${provider} | total connections: ${connections.length}, excludeId: ${excludeConnectionId || "none"}, model: ${model || "any"}`,
    );
    ...
    connections.forEach((c) => {
      ...
      if (excluded || locked) {
        ...
        log.debug(
          "AUTH",
          `  → ${c.id?.slice(0, 8)} | unavailable (${c.lastError || c.errorCode || c.testStatus})`,
        );
      }
    });
...
export async function markAccountUnavailable(
  ...
  const customCooldownMs = getProviderCooldownOverride({
    provider,
    model,
    status,
    errorText,
...
export async function clearAccountError(
  ...
  log.info(
    "AUTH",
    `Account ${connectionId.slice(0, 8)} cleared lock for model=${model || "__all"}`,
  );
}
